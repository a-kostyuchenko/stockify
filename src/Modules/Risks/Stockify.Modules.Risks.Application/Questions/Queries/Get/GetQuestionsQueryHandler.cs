...
internal sealed class GetQuestionsQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<GetQuestionsQuery, PagedResponse<QuestionResponse>>
{
    public async Task<Result<PagedResponse<QuestionResponse>>> Handle(
    ...
    )
    {
        ...
        IReadOnlyCollection<QuestionResponse> questions = await GetQuestionsAsync(
            connection,
            parameters
        );
    ...
    )
    {
        ...
        List<QuestionResponse> questions = (
            await connection.QueryAsync<QuestionResponse>(sql, parameters)
        ).AsList();

        return questions;
    }

    private static async Task<int> CountQuestionsAsync(
        DbConnection connection,
        GetQuestionsParameters parameters
    )
    {
